...
use stateset_owner::{OwnerError, OwnerInit::SetInitialOwner, OwnerUpdate};
use stateset_loan_types::{
    address_provider::{self, StatesetAddressType},
    error::StatesetError,
    loan::{
...
pub const CONTRACT_NAME: &str = "crates.io:stateset-loan";
...
pub fn instantiate(deps: DepsMut, msg: InstantiateMsg) -> Result<Response, ContractError> {
    ...
    if !available {
        return Err(StatesetError::InstantiateParamsUnavailable {}.into());
    };
        ...
        Some(mut market) => {
            ...
            if should_update_interest_rates {
                ...
                let addresses = address_provider::helpers::query_contract_addrs(
                    deps.as_ref(),
                    &config.address_provider,
                    vec![StatesetAddressType::Incentives, StatesetAddressType::RewardsCollector],
                )?;
                let rewards_collector_addr = &addresses[&StatesetAddressType::RewardsCollector];
                let incentives_addr = &addresses[&StatesetAddressType::Incentives];
...
fn update_asset_by_emergency_owner(
    ...
    if let Some(mut market) = MARKETS.may_load(deps.storage, denom)? {
        match params.borrow_enabled {
            ...
            _ => Err(StatesetError::Unauthorized {}.into()),
        }
    } else {
...
pub fn withdraw(
    ...
    let addresses = address_provider::helpers::query_contract_addrs(
        ...
        &config.address_provider,
        vec![
            StatesetAddressType::Oracle,
            StatesetAddressType::Incentives,
            StatesetAddressType::RewardsCollector,
        ],
    )?;
    let rewards_collector_addr = &addresses[&StatesetAddressType::RewardsCollector];
    let incentives_addr = &addresses[&StatesetAddressType::Incentives];
    let oracle_addr = &addresses[&StatesetAddressType::Oracle];
